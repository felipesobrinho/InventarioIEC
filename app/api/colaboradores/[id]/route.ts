...
async function enrichAuditSnapshot(
...
export async function PUT(
  ...
  // descarta campos virtuais que não existem no banco
  const {
    created_at,
    id: _id,
    setor,
  ...
  const anterior =
    await prisma.colaboradores.findUnique({
      where: { id },
    })

  const item = await prisma.colaboradores.update({
    where: { id },
    data,
  })
...
export async function DELETE(
  ...
  const anteriorEnriquecido =
    await enrichAuditSnapshot(anterior as any)

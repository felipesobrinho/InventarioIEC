...
function isAllocated(item: Notebook) {
 return (
  (item.alocacoes_ativas?.length ?? 0) > 0 || Boolean(item.alocacao_ativa)
 );
}
...
export default function NotebooksPage() {
 ...
 function applyOverviewFilter(filter: OverviewFilter) {
  ...
  const predicates: Record<
   string,
   { label: string; predicate: (item: Notebook) => boolean }
  > = {
   ...
   "notebook-borrowed": {
    ...
    predicate: (item) => item.emprestado === true,
   },
   "notebook-missing-data": {
    label: "Notebooks com informacoes faltantes",
  ...
  void Promise.resolve().then(() => {
   setActiveOverviewFilters((currentFilters) => {
    ...
    for (const sectorName of sectorNames) {
     if (nextFilters.some((filter) => filter.key === `sector:${sectorName}`))
      continue;
     nextFilters.push({
      kind: "sector",
      value: sectorName,
  ...
  () => [
    ...
    cell: ({ row }) => (
     <div>
      ...
      <p className="text-xs text-slate-400">
       {row.original.fabricante || "Sem fabricante"}
      </p>
     </div>
    ),
   },
   ...
   },
   {
    ...
    cell: ({ row }) => {
     ...
     const label =
      (nb as any).emprestado_colaborador_nome ??
...
function getNotebookSetor(item?: Notebook | null) {
 return (
  item?.setor_nome ||
  item?.setor ||
  item?.alocacao_ativa?.colaborador.setor_rel?.nome ||
  "Sem setor"
 );
}

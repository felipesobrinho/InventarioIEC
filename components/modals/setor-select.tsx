...
export function SetorSelect({
  ...
  // Nome do setor selecionado para exibição
  const selectedNome = value
    ? setores.find(s => s.id === value)?.nome ?? null
    : null

  // Buscar setores com debounce
  useEffect(() => {
    if (!open) return
  ...
  return (
    <div className={`relative ${className}`}>
      {/* Trigger */}
      {value && selectedNome ? (
        <div className={`flex items-center gap-2 px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 ${disabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}`}
          onClick={() => !disabled && setOpen(true)}
        >
          <MapPin className="w-3.5 h-3.5 text-blue-500 shrink-0" />
          <span className="flex-1 text-sm text-slate-800 dark:text-slate-200 truncate">{selectedNome}</span>
          {!disabled && (
            <button
              type="button"
        ...
        <div
          className={`flex items-center gap-2 px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 ${disabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer hover:border-slate-300 dark:hover:border-slate-600'} transition`}
            ...
            <div className="max-h-48 overflow-y-auto">
              {setores.length === 0 && !loading ? (
                <p className="text-xs text-slate-400 text-center py-4">
                  {search ? `Nenhum setor encontrado para "${search}"` : 'Nenhum setor cadastrado.'}
                </p>
              ) : (
                setores.map(s => (
                  <button
                    key={s.id}
                    type="button"
                    onClick={() => { onChange(s.id, s.nome); setOpen(false); setSearch('') }}
                    className={`w-full text-left px-3 py-2.5 text-sm flex items-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-700 transition ${value === s.id ? 'text-blue-600 dark:text-blue-400 bg-blue-50/50 dark:bg-blue-950/20' : 'text-slate-700 dark:text-slate-200'}`}
                  >
              ...
              <div className="border-t border-slate-100 dark:border-slate-700 p-2">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-2 py-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                  <button
                    type="button"
                    onClick={handleCreate}
                    disabled={!novoNome.trim() || creating}
                    className="flex items-center gap-1 px-2.5 py-1.5 text-xs bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50 transition"
                  >

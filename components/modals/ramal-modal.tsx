...
export function RamalModal({ ramal, onClose, onRefresh }: Props) {
  ...
  async function desalocar() {
    setSavingAlocacao(true)
    try {
      const res = await fetch(`/api/alocacoes/ramais/${ramal.id}/ativo`, { method: 'DELETE' })
      if (!res.ok) throw new Error()
        ...
        <aside className="w-full max-w-md bg-white dark:bg-slate-900 shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex items-start justify-between p-5 border-b border-slate-100 dark:border-slate-800">
            <div>
              <h2 className="text-base font-semibold text-slate-900 dark:text-white font-mono">
                {mode === 'edit' ? 'Editar Ramal' : `Ramal ${ramal.numero_ramal ?? '—'}`}
              </h2>
              {mode === 'view' && (
              ...
              {ramal.alocacao_ativa ? (
                <div className="bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-900 rounded-lg p-4">
                  ...
                  {ramal.alocacao_ativa.colaborador.setor && (
                    <p className="text-xs text-green-600 dark:text-green-500">{ramal.alocacao_ativa.colaborador.setor}</p>
                  )}
          ...
          {mode === 'edit' && (
            <div className="flex-1 overflow-y-auto p-5">
              <form id="ramal-form" onSubmit={(e) => { e.preventDefault(); handleSubmit(onSubmit)(e); }} noValidate>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex items-center gap-2 pt-4">
                    <input type="checkbox" id="fila-edit" {...register('fila')}
                      className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                    <label htmlFor="fila-edit" className="text-sm text-slate-700 dark:text-slate-300 cursor-pointer">Fila</label>
                  </div>
                  <div className="flex items-center gap-2 pt-4">
                    <input type="checkbox" id="cont-edit" {...register('contemplacao')}
                      className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                    <label htmlFor="cont-edit" className="text-sm text-slate-700 dark:text-slate-300 cursor-pointer">Contemplação</label>
                  </div>
                </div>
              </form>
            </div>
          ...
          <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex gap-2">
            {mode === 'view' ? (
              ...
              <>
                ...
                <button type="submit" form="ramal-form" disabled={saving}

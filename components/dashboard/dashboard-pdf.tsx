...
const s = StyleSheet.create({
  page: { fontFamily: 'Inter', backgroundColor: c.white, padding: 32, fontSize: 9 },
  // Cabeçalho
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: c.slate200 },
  headerLeft: { flex: 1 },
  headerTitle: { fontSize: 20, fontWeight: 700, color: c.slate900 },
  headerSub: { fontSize: 9, color: c.slate400, marginTop: 2 },
  headerBadge: { backgroundColor: c.slate100, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  ...
  metricCard: { flex: 1, backgroundColor: c.slate50, borderWidth: 1, borderColor: c.slate100, borderRadius: 8, padding: 8 },
  metricLabel: { fontSize: 7, color: c.slate400, fontWeight: 600, textTransform: 'uppercase', marginBottom: 3 },
  ...
  // Tabela
  table: { borderWidth: 1, borderColor: c.slate100, borderRadius: 8, overflow: 'hidden', marginTop: 6 },
  tableHeader: { flexDirection: 'row', backgroundColor: c.slate50, paddingVertical: 5, paddingHorizontal: 8, borderBottomWidth: 1, borderBottomColor: c.slate100 },
  tableHeaderCell: { fontSize: 7, fontWeight: 700, color: c.slate400, textTransform: 'uppercase' },
  tableRow: { flexDirection: 'row', paddingVertical: 5, paddingHorizontal: 8, borderBottomWidth: 1, borderBottomColor: c.slate50 },
  tableRowAlt: { flexDirection: 'row', paddingVertical: 5, paddingHorizontal: 8, borderBottomWidth: 1, borderBottomColor: c.slate50, backgroundColor: '#fafafa' },
  ...
  // Badge alocado
  badgeGreen: { backgroundColor: c.green100, paddingHorizontal: 5, paddingVertical: 1, borderRadius: 4 },
  badgeGreenText: { fontSize: 7, color: c.green700, fontWeight: 600 },
  // Setores
  setoresGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 4, marginTop: 6 },
  setorCard: { backgroundColor: c.slate50, borderWidth: 1, borderColor: c.slate100, borderRadius: 6, padding: 6, width: '31%' },
  setorName: { fontSize: 7, fontWeight: 600, color: c.slate700, marginBottom: 2 },
  setorMeta: { fontSize: 7, color: c.slate400 },
  progressBarBg: { backgroundColor: c.slate200, borderRadius: 4, height: 4, marginTop: 3 },
  // Footer
  footer: { position: 'absolute', bottom: 20, left: 32, right: 32, flexDirection: 'row', justifyContent: 'space-between', borderTopWidth: 1, borderTopColor: c.slate100, paddingTop: 8 },
...
function buildSetores(items: any[]) {
  ...
  for (const item of items) {
    const setor = item.setor || item.nome_setor || item.alocacao_ativa?.colaborador?.setor || 'Sem setor'
    const cur = map.get(setor) ?? { total: 0, alocados: 0 }
    cur.total++
    if ((item.alocacoes_ativas?.length ?? 0) > 0 || item.alocacao_ativa) cur.alocados++
    map.set(setor, cur)
  }
  return Array.from(map.entries())
    .map(([setor, v], i) => ({ setor, ...v, color: CHART_COLORS[i % CHART_COLORS.length] }))
...
function Section({ title, color, items, total, getIdentifier, getModelo, getSetor, getExtra }: SectionProps) {
  const alocados = items.filter(i => (i.alocacoes_ativas?.length ?? 0) > 0 || i.alocacao_ativa).length
  const livres = Math.max(0, items.length - alocados)
  const ocupacao = pct(alocados, items.length)
  const setores = buildSetores(items)

  const metricValueStyle = ocupacao >= 90 ? s.metricValueRed : ocupacao >= 50 ? s.metricValueAmber : s.metricValueGreen
  ...
  return (
    <View style={s.section} break>
      ...
      <View style={s.sectionHeader}>
        ...
        <Text style={s.sectionSub}>{total.toLocaleString('pt-BR')} cadastrados no total</Text>
      </View>
      ...
      {setores.length > 0 && (
        <View>
          <Text style={[s.tableHeaderCell, { marginBottom: 4 }]}>Distribuição por setor</Text>
            ...
            {setores.map(st => (
              <View key={st.setor} style={s.setorCard}>
                ...
                <View style={s.progressBarBg}>
                  <View style={{ width: `${pct(st.alocados, st.total)}%` as any, backgroundColor: st.color, height: 4, borderRadius: 4 }} />
                </View>
              </View>
            ))}
          </View>
...
export function DashboardPDF({ data }: Props) {
  ...
  const totalGeral = maquinas.length + notebooks.length + aparelhos.length + ramais.length
  const totalAlocados = [
    ...maquinas, ...notebooks, ...aparelhos, ...ramais,
  ].filter(i => (i.alocacoes_ativas?.length ?? 0) > 0 || i.alocacao_ativa).length
  ...
  return (
    <Document title="Dashboard IEC — Inventário de TI" author="IEC Inventário" creator="IEC Inventário">
      ...
      <Page size="A4" style={s.page}>
        <View style={s.header}>
          <View style={s.headerLeft}>
            ...
            <Text style={[s.headerSub, { fontSize: 11, color: c.slate700, marginTop: 3 }]}>
        ...
        <View style={{ marginBottom: 20 }}>
          <Text style={[s.tableHeaderCell, { marginBottom: 8, fontSize: 8 }]}>Resumo executivo</Text>
          <View style={s.metricsRow}>
            <View style={s.metricCard}>
            ...
            <View style={s.metricCard}>
              ...
              <Text style={s.metricValue}>{totalAlocados.toLocaleString('pt-BR')}</Text>
            </View>
            <View style={s.metricCard}>
              <Text style={s.metricLabel}>Livres</Text>
              <Text style={s.metricValueGreen}>{(totalGeral - totalAlocados).toLocaleString('pt-BR')}</Text>
            </View>
            <View style={s.metricCard}>
              <Text style={s.metricLabel}>Ocupação geral</Text>
              <Text style={[pct(totalAlocados, totalGeral) >= 80 ? s.metricValueAmber : s.metricValueGreen]}>
        ...
        {/* Sumário */}
        <View style={{ borderWidth: 1, borderColor: c.slate100, borderRadius: 8, overflow: 'hidden' }}>
          ...
          {[
            ...
            const aloc = items.filter(i => (i.alocacoes_ativas?.length ?? 0) > 0 || i.alocacao_ativa).length
            ...
            return (
              <View key={label} style={idx % 2 === 0 ? s.tableRow : s.tableRowAlt}>
                ...
                <Text style={[s.tableCell, { flex: 1 }]}>{items.length.toLocaleString('pt-BR')}</Text>
                <Text style={[s.tableCell, { flex: 1 }]}>{aloc.toLocaleString('pt-BR')}</Text>
                <Text style={[s.metricValueGreen, { flex: 1, fontSize: 8 }]}>{livres.toLocaleString('pt-BR')}</Text>
                <Text style={[ocp >= 80 ? s.metricValueAmber : s.metricValueGreen, { flex: 1, fontSize: 8 }]}>{ocp}%</Text>
              </View>
            )
          })}
        </View>
      ...
      <Page size="A4" style={s.page}>
        <Section
          ...
          getModelo={i => [i.fabricante, i.modelo].filter(Boolean).join(' ') || '—'}
          getSetor={i => i.setor || '—'}
          getExtra={i => i.categoria || '—'}
        />
        <View style={s.footer} fixed>
          <Text style={s.footerText}>IEC Inventário TI — Máquinas</Text>
          <Text style={s.pageNumber} render={({ pageNumber, totalPages }) => `Pág. ${pageNumber} / ${totalPages}`} />
        </View>
      </Page>
      ...
      <Page size="A4" style={s.page}>
        ...
        />
        <View style={s.footer} fixed>
          <Text style={s.footerText}>IEC Inventário TI — Notebooks</Text>
          <Text style={s.pageNumber} render={({ pageNumber, totalPages }) => `Pág. ${pageNumber} / ${totalPages}`} />
        </View>
      </Page>
      ...
      <Page size="A4" style={s.page}>
        ...
        />
        <View style={s.footer} fixed>
          <Text style={s.footerText}>IEC Inventário TI — Aparelhos</Text>
          <Text style={s.pageNumber} render={({ pageNumber, totalPages }) => `Pág. ${pageNumber} / ${totalPages}`} />
        </View>
      </Page>
      ...
      <Page size="A4" style={s.page}>
        <Section
          ...
          total={ramais.length}
          geradoEm={gerado_em}
          getIdentifier={i => i.numero_ramal != null ? String(i.numero_ramal) : '—'}
          getModelo={i => i.nome_setor || '—'}
          getSetor={i => i.prefixo_telefonico || '—'}
          getExtra={i => i.fila ? 'Fila' : i.contemplacao ? 'Contemplado' : '—'}
        />
        <View style={s.footer} fixed>
          <Text style={s.footerText}>IEC Inventário TI — Ramais</Text>
          <Text style={s.pageNumber} render={({ pageNumber, totalPages }) => `Pág. ${pageNumber} / ${totalPages}`} />
        </View>
      </Page>
